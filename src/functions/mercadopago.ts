...
export async function createMercadoPagoCharge(
  ...
  const sanitizedDescription =
    params.description.trim().substring(0, 200) || "Serviço / Atendimento";
  ...
  try {
    ...
    let cardCheckout: { preferenceId: string; initPoint: string } | undefined =
      undefined;
    ...
    try {
      ...
      if (prefResponse.ok) {
        ...
        console.warn(
          "[MercadoPago] Falha ao criar preferência de cartão:",
          await prefResponse.text(),
        );
      }
    } catch (err) {
