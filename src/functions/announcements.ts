...
export interface AnnouncementContainerOptions {
  title: string;
  message: string;
  color?: string;
  image?: string;
...
export function announcementContainer(options: AnnouncementContainerOptions) {
  const finalColor =
  ...
  if (options.image || options.thumbnail) {
    items.push(
      createSection({
        ...
        thumbnail: (options.thumbnail || options.image) as any,
      }),
    );
  } else {
...
export interface AnnouncementDMOptions {
  guildName: string;
  title: string;
  message: string;
  color?: string;
...
export function announcementDMContainer(options: AnnouncementDMOptions) {
  const finalColor =
    options.color && /^#?[0-9a-fA-F]{3,8}$/.test(options.color)
      ? formatHexColor(options.color)
      : formatHexColor("#38bdf8");
...
export async function resolveRoleRecipients(
  ...
  const roleSet = roleIds instanceof Set ? roleIds : new Set<string>([...roleIds]);

  const fromCache = guild.members.cache.filter(
    (m) => !m.user.bot && [...roleSet].some((id) => m.roles.cache.has(id)),
  );
...
export interface SendChannelAnnouncementOptions {
  channel: TextChannel;
  title: string;
  message: string;
  color?: string;
...
export async function sendChannelAnnouncement(
  options: SendChannelAnnouncementOptions,
) {
  const {
    channel,
    title,
...
export interface SendDMAnnouncementOptions {
  guild: Guild;
  role: Role | Role[];
  title: string;
  message: string;
