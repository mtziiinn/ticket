...
const pendingPixCodes = new Map<string, { payload: string; createdAt: number }>();

export function registerPixCode(payload: string): string {
  const id = Math.random().toString(36).slice(2, 10);
  pendingPixCodes.set(id, { payload, createdAt: Date.now() });
  return id;
}

export function getPixCode(id: string): string | undefined {
  return pendingPixCodes.get(id)?.payload;
}

export function cleanupPendingPixCodes(): number {
  const now = Date.now();
  let cleaned = 0;
