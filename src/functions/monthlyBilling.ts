 ...
 */
export function buildMonthlyBillingContainer(
  user: User | { id: string },
  clientName: string,
  monthYear: string,
  pixPayload: string,
  ...
  const container = createContainer(
    ...
    Separator.Default,
    `*Após efetuar o pagamento, por gentileza envie o comprovante para <@${MONTHLY_BILLING_CONFIG.developerId}> (@mtzin7.rp) para confirmação da renovação mensal.*`,
  );
...
export async function sendMonthlyBillingDM(
  ...
  try {
    ...
    const pixPayload = generatePixPayload(
      MONTHLY_BILLING_CONFIG.pixKey,
      MONTHLY_BILLING_CONFIG.beneficiaryName,
      "SAO PAULO",
      MONTHLY_BILLING_CONFIG.amount,
    );

    const container = buildMonthlyBillingContainer(
      user,
      clientConfig.name,
      monthYear,
      pixPayload,
