...
createResponder({
  ...
  async run(interaction) {
    ...
    const targetChannelId =
      guildData.channels?.general || guildData.channels?.tickets;
    ...
    const payload = buildCustomOrFallbackPayload(
      guildData?.customPanels?.ticket,
      container,
      [actionRow],
    );
...
createResponder({
  ...
  async run(interaction) {
    ...
    const parentSelect = new ChannelSelectMenuBuilder()
      ...
      .setPlaceholder("Selecione a categoria de destino")
      .setChannelTypes(ChannelType.GuildCategory);
    if (cat.parentId) parentSelect.setDefaultChannels(cat.parentId);
...
createResponder({
  ...
  async run(interaction, { oldValue }) {
    ...
    const index = list.findIndex((c: any) => (c.value || c.name) === oldValue);
    ...
    if (guildData.channels?.ticketCategories) {
      guildData.channels.ticketCategories =
        guildData.channels.ticketCategories.filter(
          (c: any) => c.value !== selected && c.name !== selected,
        );
      guildData.markModified("channels");
      await (guildData as any).save();
    }
...
createResponder({
  ...
  async run(interaction) {
    ...
    const guildData = await db.guilds.get(interaction.guild.id);
    if (!guildData.payments) guildData.payments = {} as any;
...
createResponder({
  ...
  async run(interaction) {
    ...
    const mpPublicKey =
      interaction.fields.getTextInputValue("mpPublicKey") || "";
...
createResponder({
  ...
  async run(interaction) {
    const stripeSecretKey =
      interaction.fields.getTextInputValue("stripeSecretKey");
    const stripeWebhookSecret =
      interaction.fields.getTextInputValue("stripeWebhookSecret") || "";
...
createResponder({
  ...
  async run(interaction) {
    ...
    const guildData = await db.guilds.get(interaction.guild.id);
    if (!guildData.welcome) guildData.welcome = {} as any;
...
createResponder({
  ...
  async run(interaction) {
    ...
    const label = new LabelBuilder()
      ...
      .setDescription(
        "Ex: 3 (contas criadas há menos de 3 dias serão barradas)",
      )
      .setTextInputComponent(
        new TextInputBuilder()
          .setCustomId("days")
...
createResponder({
  ...
  async run(interaction) {
    ...
    const payload = buildCustomOrFallbackPayload(
      guildData?.customPanels?.verification,
      container,
      [verifyRow],
    );
...
createResponder({
  ...
  async run(interaction) {
    ...
    const guildData = await db.guilds.get(interaction.guild.id);
    if (!guildData.verification) guildData.verification = {} as any;
  ...
  async run(interaction) {
    ...
    modal.addComponents(
      new LabelBuilder()
        ...
        .setTextInputComponent(mentionsInput),
      new LabelBuilder()
        .setLabel("Janela de Tempo (Segundos):")
        .setTextInputComponent(windowInput),
      new LabelBuilder()
...
createResponder({
  ...
  cache: "cached",
  async run(interaction) {
    const rawMentions = parseInt(
      interaction.fields.getTextInputValue("max_mentions").trim(),
      10,
    );
    const rawWindow = parseInt(
      interaction.fields.getTextInputValue("window_seconds").trim(),
      10,
    );
    const rawTimeout = parseInt(
      interaction.fields.getTextInputValue("timeout_minutes").trim(),
      10,
    );
