...
createResponder({
  ...
  async run(interaction) {
    ...
    if (!guildData.announcements) guildData.announcements = {};
...
createResponder({
  ...
  async run(interaction) {
    ...
    let targetChannel =
      (targetId &&
        (interaction.guild.channels.cache.get(targetId) as TextChannel | undefined)) ||
      undefined;
    ...
    try {
      ...
      const errContainer = createContainer(
        formatHexColor("#ef4444"),
        `## ${getEmojiTag("action_x")} Falha ao Enviar Comunicado`,
        Separator.Default,
        `Ocorreu um erro ao enviar o anúncio: \`${err.message}\``,
      );
      await interaction.editReply({
...
createResponder({
  ...
  async run(interaction) {
    ...
    const roles = roleIds
      .map((id: string) => interaction.guild.roles.cache.get(id) as Role | undefined)
    ...
    try {
      ...
      if (recipients === 0) {
        const errContainer = createContainer(
          formatHexColor("#ef4444"),
          `## ${getEmojiTag("action_warning")} Nenhum Membro Encontrado`,
          Separator.Default,
          `Nenhum membro ativo possui algum dos cargos selecionados.`,
        );
        await interaction.editReply({
          components: [errContainer],
          flags: ["IsComponentsV2"] as any,
      ...
      const errContainer = createContainer(
        formatHexColor("#ef4444"),
        `## ${getEmojiTag("action_x")} Erro no Processamento`,
        Separator.Default,
        `Ocorreu um erro inesperado: \`${err.message}\``,
      );
      await interaction.editReply({
        components: [errContainer],
        flags: ["IsComponentsV2"] as any,
...
async function extractJsonContent(
  ...
  let rawContent = "";
  try {
    const uploadedFiles = (interaction.fields as any).getUploadedFiles?.("json_file");
    const attachment =
      uploadedFiles?.first?.() ||
...
createResponder({
  ...
  async run(interaction) {
    ...
    try {
      ...
      const errContainer = createContainer(
        formatHexColor("#ef4444"),
        `## ${getEmojiTag("action_x")} Falha no Envio`,
        Separator.Default,
        `Ocorreu um erro ao enviar o anúncio: \`${err.message}\``,
      );
      await interaction.editReply({
...
createResponder({
  ...
  async run(interaction) {
    ...
    const roleIdSet = new Set(roles.map((r: Role) => r.id));
