...
createEvent({
  ...
  async run(message: Message) {
    ...
    try {
      ...
      const mentionedStaffRole =
        Boolean(staffRoleId) && message.mentions.roles.has(staffRoleId!);

      // 2. Verificar se houve menção a membros que possuem o cargo da staff
      const mentionedStaffMember =
        message.mentions.members &&
        message.mentions.members.some((m) => {
          if (m.id === message.author.id) return false;
          if (staffRoleId && m.roles.cache.has(staffRoleId)) return true;
      ...
      const hasStaffMention =
        mentionedStaffRole || mentionedStaffMember || mentionedClaimer;

      if (!hasStaffMention) return;
      ...
      if (timestamps.length >= maxMentions) {
        ...
        const timeoutApplied = await message.member
          ...
          .catch((err) => {
            console.error(
              `[Anti-Flood] Erro ao aplicar timeout em ${message.author.id}:`,
              err,
            );
            return false;
          });

        if (!timeoutApplied) return;

        // Container de aviso no canal
        const warningContainer = createContainer(
          "#ef4444",
          createSection({
            ...
            thumbnail: message.author.displayAvatarURL() as any,
          }),
          Separator.Default,
          [
          ...
          Separator.Default,
          `*Por favor, aguarde o atendimento da equipe sem ficar marcando os membros repetidamente.*`,
        );
