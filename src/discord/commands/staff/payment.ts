...
createCommand({
  ...
  description:
    "💳 Gera cobrança para um cliente usando suas credenciais (/meu-pagamento) ou as da loja.",
  ...
  const clientMention = finalTargetUserId
    ...
    if (gateway === "pix_manual") {
      ...
      if (ticket) {
        ...
        await sendActionLog(
          ...
          interaction.user,
          "Gerar Cobrança",
          `Gerou cobrança no valor de **${formattedAmount}** via PIX Manual (${isIndividual ? "Chave Pessoal" : "Loja"}).`,
        );
    ...
    if (gateway === "pix_mp" || gateway === "card_mp") {
      ...
      const container = createContainer(
        ...
        Separator.Default,
        ...mediaItems,
        mediaItems.length > 0 ? Separator.Default : [],
        ...rows,
        rows.length > 0 ? Separator.Default : [],
        `*O pagamento é verificado automaticamente pelo sistema assim que for aprovado.*`,
      );
      ...
      if (ticket) {
        ...
        await sendActionLog(
          ...
          interaction.user,
          "Gerar Cobrança",
          `Gerou cobrança no valor de **${formattedAmount}** via Mercado Pago (${isIndividual ? "Conta Pessoal" : "Loja"}).`,
        );
    ...
    const container = createContainer(
      ...
      ...rows,
      rows.length > 0 ? Separator.Default : [],
      `*Acesse a fatura pelo link de checkout seguro da Stripe.*`,
    );
    ...
    if (ticket) {
      ...
      await sendActionLog(
        ...
        interaction.user,
        "Gerar Cobrança",
        `Gerou cobrança no valor de **${formattedAmount}** via Stripe (${isIndividualStripe ? "Conta Pessoal" : "Loja"}).`,
      );
