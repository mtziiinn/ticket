...
function markUsed(id) {
    ...
    if (usedEntryIds.size > MAX_USED_ENTRIES) {
        ...
        if (oldest !== undefined)
            usedEntryIds.delete(oldest);
    }
}
function newestEntry(logs, memberId, now, maxAgeMs, wantsKey) {
    if (!logs?.entries)
        return null;
    let best = null;
    for (const entry of logs.entries.values()) {
        if (entry.targetId !== memberId)
            continue;
        if (now - entry.createdTimestamp >= maxAgeMs)
            continue;
        if (usedEntryIds.has(entry.id))
            continue;
        if (wantsKey && !entry.changes?.some((c) => c.key === wantsKey))
            continue;
        if (!best || entry.createdTimestamp > best.createdTimestamp)
            best = entry;
    }
    return best;
}
...
export async function getMemberChangesFromAuditLog(guild, memberId, { maxAgeMs = 15_000, waitMs = 1_200 } = {}) {
    try {
        if (waitMs > 0)
        ...
        const now = Date.now();
        const result = { added: [], removed: [], executor: null };
        let found = false;
        ...
        if (nickEntry) {
            const change = nickEntry.changes.find((c) => c.key === "nick");
