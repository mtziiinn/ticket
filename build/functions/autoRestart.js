...
function readFileValue(file, key) {
    try {
...
async function restartSelf(appId, token) {
    try {
...
export function startAutoRestart() {
    ...
    if (env.DISCLOUD_APP_ID && env.DISCLOUD_APP_ID !== appId) {
        log.warn("AutoRestart", `DISCLOUD_APP_ID do ambiente (${env.DISCLOUD_APP_ID}) difere do ID deste app (${appId}); usando o do discloud.config.`);
    }
