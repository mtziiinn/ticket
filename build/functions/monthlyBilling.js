 ...
 */
export function buildMonthlyInvoiceContainer(user, clientName, monthYear) {
    ...
    const container = createContainer(brandColor, `## ${getEmojiTag("other_dollar") || "🟢"} Fatura Mensal • Manutenção & Hospedagem`, Separator.Default, [
        ...
        `> \`/pagar-mensalidade\``,
    ].join("\n"), Separator.Default, createRow(generateBtn), Separator.Default, `*Após a confirmação do pagamento, a renovação da hospedagem é computada automaticamente no sistema.*`);
    return container;
}
/**
 ...
 * mensagem nova.
 */
export function buildMonthlyReminderContainer(user, clientName, monthYear, daysSinceSent) {
    const [year, month] = monthYear.split("-");
    const formattedPeriod = `${month}/${year}`;
    ...
    const generateBtn = new ButtonBuilder()
        ...
        .setStyle(ButtonStyle.Success)
        .setEmoji(getEmojiId("other_card") || "💳");
    const container = createContainer(brandColor, `## ${getEmojiTag("clock") || "⏰"} Lembrete • Fatura ${formattedPeriod} em aberto`, Separator.Default, [
        `Oi <@${user.id}>! A fatura de manutenção e hospedagem referente a **${formattedPeriod}** ainda consta **em aberto** há ${daysSinceSent} dias.`,
        "",
        `| **Cliente:** <@${user.id}> (${clientName})`,
        `| **Valor:** \`R$ ${MONTHLY_BILLING_CONFIG.amount.toFixed(2).replace(".", ",")}\``,
        `| **Beneficiário:** ${MONTHLY_BILLING_CONFIG.beneficiaryName} (<@${MONTHLY_BILLING_CONFIG.developerId}>)`,
    ].join("\n"), Separator.Default, createRow(generateBtn), Separator.Default, `*Se já pagou, ignore — a confirmação pode levar alguns minutos pra chegar. Qualquer dúvida, chama o desenvolvedor.*`);
    return container;
}
/**
 * Constrói o container visual com os dados do PIX gerado pelo Mercado Pago.
 */
export function buildMonthlyPixContainer(user, clientName, monthYear, pix) {
    const [year, month] = monthYear.split("-");
    const formattedPeriod = `${month}/${year}`;
    ...
    const container = createContainer(brandColor, `## ${getEmojiTag("other_dollar") || "🟢"} Pagamento PIX • Fatura ${formattedPeriod}`, Separator.Default, [
        `| **Cliente:** <@${user.id}> (${clientName})`,
 ...
 */
export function buildMonthlyPaidContainer(user, clientName, monthYear, amount, paidAt = new Date()) {
    const [year, month] = monthYear.split("-");
    const formattedPeriod = `${month}/${year}`;
    const container = createContainer("#22c55e", `## ${getEmojiTag("action_check") || "✅"} Pagamento Confirmado • Fatura ${formattedPeriod}`, Separator.Default, [
    ...
    ].join("\n"), Separator.Default, `*A hospedagem e a manutenção do seu bot estão garantidas. Muito obrigado pela preferência e parceria!*`);
    return container;
}
/**
 * Gera um pagamento PIX dinâmico no Mercado Pago para a mensalidade.
 */
export async function generateMonthlyPixPayment(userId, monthYear = getCurrentMonthYear()) {
    let clientConfig = MONTHLY_BILLING_CONFIG.clients.find((c) => c.id === userId);
    ...
    const botName = typeof brand !== "undefined" && brand.brandName
        ? brand.brandName
        : "Discord Bot";
    try {
        ...
        await db.monthlyBillings.findOneAndUpdate({ userId, monthYear }, {
            userId,
        ...
        if (mpStatus === "approved") {
            ...
            if (billing && billing.status !== "paid") {
                ...
                try {
                    ...
                    if (user) {
                        const paidContainer = buildMonthlyPaidContainer(user, billing.clientName, billing.monthYear, billing.amount, paidAt);
                        let updated = false;
                ...
                try {
                    ...
                    if (devUser) {
                        ...
                        const devNotice = createContainer("#22c55e", `## ${getEmojiTag("action_check") || "🎉"} Mensalidade Recebida!`, Separator.Default, [
 ...
 */
export function startPaymentPolling(paymentId, client) {
    let attempts = 0;
...
export async function sendMonthlyBillingDM(client, clientConfig, options = {}) {
    try {
        ...
        const container = buildMonthlyInvoiceContainer(user, clientConfig.name, monthYear);
        let dmMsg = null;
        try {
 ...
 */
export async function checkPendingMonthlyPayments(client) {
    try {
 ...
 */
export async function checkAndSendPaymentReminders(client) {
    try {
        const brasiliaDate = getBrasiliaDate();
        if (brasiliaDate.getDate() < MONTHLY_BILLING_CONFIG.reminderDay)
            return;
        ...
        for (const billing of pendingBillings) {
            ...
            if (!user)
                continue;
            const daysSinceSent = Math.floor((Date.now() - billing.sentAt.getTime()) / (24 * 60 * 60 * 1000));
            const reminderContainer = buildMonthlyReminderContainer(user, billing.clientName, billing.monthYear, daysSinceSent);
            const sent = await safeSendDM(user, {
            ...
            if (sent) {
                ...
                await billing.save();
                console.log(`[Monthly Billing] Lembrete enviado para ${billing.clientName} (${billing.userId}) — ${billing.reminderCount}º lembrete.`);
            }
        }
    }
    catch (error) {
 ...
 */
export async function checkAndEnforceShutdown(client) {
    try {
        if (MONTHLY_BILLING_CONFIG.clients.length === 0)
            return;
        const brasiliaDate = getBrasiliaDate();
        if (brasiliaDate.getDate() < MONTHLY_BILLING_CONFIG.shutdownDay)
            return;
        const currentMonthYear = getCurrentMonthYear(brasiliaDate);
        ...
        console.error(`[Monthly Billing] Nenhum cliente pagou ate o dia ${MONTHLY_BILLING_CONFIG.shutdownDay} — desligando o bot.`);
        try {
            const devUser = await client.users
                .fetch(MONTHLY_BILLING_CONFIG.developerId)
                .catch(() => null);
            if (devUser) {
                const clientList = MONTHLY_BILLING_CONFIG.clients
                    .map((c) => `<@${c.id}> (${c.name})`)
                    .join(", ");
                const shutdownNotice = createContainer("#ef4444", `## ${getEmojiTag("action_x") || "🔴"} Bot desligado por falta de pagamento`, Separator.Default, [
                ...
                ].join("\n"), Separator.Default, `*Assim que o pagamento for confirmado, reinicie o bot manualmente no Discloud.*`);
                await safeSendDM(devUser, { components: [shutdownNotice], flags: ["IsComponentsV2"] }, "Bot desligado — mensalidade em atraso");
            }
        }
        catch (notifyErr) {
            console.error("[Monthly Billing] Erro ao avisar dev do desligamento:", notifyErr);
        }
        client.destroy();
        process.exit(0);
    }
