...
createEvent({
    ...
    async run(emoji) {
        try {
            ...
            const timestamp = Math.floor(Date.now() / 1000);
            const container = createContainer("#ef4444", `## ${getEmojiTag("action_x")} Emoji Excluído`, Separator.Default, [
...
createEvent({
    ...
    async run(oldEmoji, newEmoji) {
        ...
        try {
            ...
            const container = createContainer("#eab308", `## ${getEmojiTag("action_info")} Emoji Renomeado`, Separator.Default, [
