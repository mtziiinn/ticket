...
createEvent({
    ...
    async run(oldChannel, newChannel) {
        ...
        try {
            ...
            const oldTopic = oldChannel.topic || "*Nenhum*";
            const newTopic = newChannel.topic || "*Nenhum*";
            ...
            const oldRate = oldChannel.rateLimitPerUser ?? 0;
            const newRate = newChannel.rateLimitPerUser ?? 0;
            if (oldRate !== newRate) {
                changes.push(`• **Modo Lento:** \`${oldRate}s\` ➔ \`${newRate}s\``);
            }
            if (changes.length === 0)
                return;
            const executor = await getAuditLogExecutor(newChannel.guild, AuditLogEvent.ChannelUpdate, newChannel.id);
            const timestamp = Math.floor(Date.now() / 1000);
            const container = createContainer("#eab308", `## ${getEmojiTag("action_info")} Canal Atualizado`, Separator.Default, [
            ...
            ].join("\n"), Separator.Default, `### Alterações Detectadas:\n${changes.join("\n")}`);
