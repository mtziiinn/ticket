...
import { MONTHLY_BILLING_CONFIG, getCurrentMonthYear, generateMonthlyPixPayment, buildMonthlyPixContainer, buildMonthlyPaidContainer, verifyMonthlyPayment, startPaymentPolling, getEmojiTag, } from "#functions";
// ==========================================
...
createCommand({
    ...
    async run(interaction) {
        const isAuthorized = MONTHLY_BILLING_CONFIG.clients.some((c) => c.id === interaction.user.id) ||
        ...
        if (existing?.status === "paid") {
            const paidContainer = buildMonthlyPaidContainer(interaction.user, existing.clientName, monthYear, existing.amount, existing.paidAt || new Date());
            await interaction.reply({
        ...
        const container = buildMonthlyPixContainer(interaction.user, res.pix.clientName, monthYear, res.pix);
        const msg = await interaction.editReply({
            components: [container],
            flags: ["IsComponentsV2"],
        });
        if (msg) {
            await db.monthlyBillings.updateOne({ mpPaymentId: res.pix.paymentId }, {
                $set: {
                    dmMessageId: msg.id,
...
createResponder({
    ...
    async run(interaction, params) {
        ...
        if (result.paid) {
            ...
            const paidContainer = buildMonthlyPaidContainer(interaction.user, billing?.clientName || interaction.user.username, billing?.monthYear || getCurrentMonthYear(), billing?.amount || MONTHLY_BILLING_CONFIG.amount, billing?.paidAt || new Date());
