    ...
    async run(interaction) {
        ...
        if (subcommand === "limpar-cache") {
            ...
            if (result.pendingPixCodesSwept > 0) {
                resourcesList.push(`${getEmojiTag("other_dollar")} **Códigos PIX expirados:** \`${result.pendingPixCodesSwept}\``);
            }
        ...
        if (subcommand === "ram") {
            ...
            const container = createContainer(constants.colors.azoxo, createSection({
                ...
                thumbnail: (emojis.static.prism || interaction.client.user?.displayAvatarURL()),
            }), Separator.Default, `### ${getEmojiTag("clock")} Tempo de Atividade (Uptime)`, `| ${getEmojiTag("clock_add")} **Uptime do Processo:** \`${uptimeStr}\`\n| ${getEmojiTag("prism")} **Bot:** \`${brand.brandName} Tickets\` | **Node.js:** \`${process.version}\` (\`${process.platform}\`)`, Separator.Default, `### ${getEmojiTag("database_check")} Alocação de Memória (RAM)`, [
            ...
            ].join("\n"), Separator.Default, `*Dica: Use \`/ticket limpar-cache\` se desejar forçar a reciclagem imediata e Garbage Collection.*`);
            await interaction.editReply({
                components: [container],
                flags: ["IsComponentsV2"],
            });
