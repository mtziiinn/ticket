...
export function createPaymentModal(targetUserId = "none", preferredGateway = "pix_manual") {
    ...
    const gatewaySelect = new StringSelectMenuBuilder()
        ...
        .setDefault(preferredGateway === "pix_manual"), new StringSelectMenuOptionBuilder()
        .setLabel("PIX (Mercado Pago)")
        .setValue("pix_mp")
        .setEmoji(getEmojiId("other_dollar") || "🟢")
        .setDescription("PIX dinâmico com aprovação automática")
        .setDefault(preferredGateway === "pix_mp"), new StringSelectMenuOptionBuilder()
        .setLabel("Cartão/Boleto (Mercado Pago)")
        .setValue("card_mp")
        .setEmoji(getEmojiId("other_card") || "🟢")
        .setDescription("Checkout transparente via Mercado Pago")
        .setDefault(preferredGateway === "card_mp"), new StringSelectMenuOptionBuilder()
...
function getSafeModalSelect(interaction, customId) {
    try {
...
function getSafeModalText(interaction, customId) {
    try {
...
async function handlePaymentModalSubmit(interaction, rawTargetUserId) {
    const memberDoc = await db.members.get({
        id: interaction.user.id,
        guild: { id: interaction.guild.id },
    });
    ...
    const cleanTargetUserId = rawTargetUserId && rawTargetUserId !== "none" ? rawTargetUserId : "";
    ...
    if (gateway === "pix_manual") {
        ...
        const receiverTag = isIndividual
            ...
            : pixName
                ...
                : `*Loja Oficial (Padrão)*`;
        const container = createContainer(constants.colors.primary, `## ${getEmojiTag("other_dollar")} Cobrança Gerada`, Separator.Default, [
    ...
    const receiverTagStripe = isIndividualStripe
        ...
        : `*Loja Oficial (Padrão)*`;
    const container = createContainer(constants.colors.primary, `## ${getEmojiTag("other_card")} Cobrança Gerada (Stripe)`, Separator.Default, [
        `| **Cliente:** ${clientMention}`,
