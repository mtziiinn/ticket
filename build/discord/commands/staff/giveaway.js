...
export function pickRandomWinners(participants, count) {
    if (participants.length === 0)
        return [];
    const shuffled = [...participants].sort(() => Math.random() - 0.5);
    return shuffled.slice(0, Math.min(count, shuffled.length));
}
export async function finishGiveaway(giveawayDoc, client) {
    giveawayDoc.ended = true;
    const winners = pickRandomWinners(giveawayDoc.participants || [], giveawayDoc.winnersCount || 1);
    ...
    try {
        ...
        if (channel && channel.isTextBased()) {
            ...
            const updatedContainer = createContainer(constants.colors.primary, `## ${getEmojiTag("prism")} Sorteio Finalizado: ${giveawayDoc.item}`, Separator.Default, `| **Ganhadores (${winners.length}):**\n${winnersText}`, Separator.Default, `*Sorteio encerrado em <t:${Math.floor(Date.now() / 1000)}:R>*`);
            if (msg) {
                await msg.edit({
            ...
            if (winners.length > 0) {
                const announceContainer = createContainer(constants.colors.primary, `| ${getEmojiTag("action_check")} **Parabéns ${winnersText}!**\nVocê(s) ganhou/ganharam **${giveawayDoc.item}** no sorteio!`);
                await channel.send({
...
createCommand({
    ...
    async run(interaction) {
        ...
        const finalContainer = createContainer(constants.colors.primary, `## ${getEmojiTag("prism")} Sorteio: ${item}`, Separator.Default, `| **Quantidade de Ganhadores:** \`${winnersCount}\`\n**Encerramento:** <t:${endTimestamp}:F> (<t:${endTimestamp}:R>)`, Separator.Default, `| Para participar, clique no botão abaixo!`, Separator.Default, createRow(new ButtonBuilder()
...
createResponder({
    ...
    async run(interaction, { messageId }) {
        ...
        const container = createContainer(constants.colors.primary, `## ${getEmojiTag("prism")} Gerenciar Sorteio`, `Escolha uma ação abaixo:`, Separator.Default, createRow(select));
        await interaction.reply({
...
createResponder({
    ...
    async run(interaction, { messageId }) {
        ...
        if (action === "view_participants") {
            ...
            const container = createContainer(constants.colors.primary, `## ${getEmojiTag("user")} Participantes do Sorteio (${participants.length})`, Separator.Default, list);
            await interaction.update({
                components: [container],
                flags: ["IsComponentsV2"],
            });
        ...
        if (action === "finish_now") {
            ...
            await interaction.update({
                components: [
                    createContainer(constants.colors.primary, `| ${getEmojiTag("action_check")} **Sorteio encerrado com sucesso.**`),
                ],
                flags: ["IsComponentsV2"],
            });
            return;
        ...
        if (action === "reroll") {
            ...
            if (channel && channel.isTextBased()) {
                const announceContainer = createContainer(constants.colors.primary, `| ${getEmojiTag("clock")} **Novo Sorteio (Reroll):**\nParabéns ${winnersText}! Você(s) foi(ram) sorteado(s) para **${giveaway.item}**!`);
                await channel.send({
                    components: [announceContainer],
                    flags: ["IsComponentsV2"],
                });
            ...
            await interaction.update({
                components: [
                    createContainer(constants.colors.primary, `| ${getEmojiTag("action_check")} **Reroll realizado com sucesso:** ${winnersText}`),
                ],
                flags: ["IsComponentsV2"],
            });
            return;
