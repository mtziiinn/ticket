...
import { ApplicationCommandType, ButtonBuilder, ButtonStyle, LabelBuilder, ModalBuilder, PermissionFlagsBits, TextInputBuilder, TextInputStyle, } from "discord.js";
 ...
 */
function maskSecret(val, keepStart = 4, keepEnd = 4) {
    if (!val || val.trim().length === 0)
        return "*Não configurado (usa da loja)*";
    const str = val.trim();
 ...
 */
export async function renderMyPaymentContainer(userId, guildId, client) {
    ...
    const stripeDisplay = hasIndividualStripe
        ...
        : guildP.stripeSecretKey
            ...
            : `\`Não configurada\` • ${getEmojiTag("action_x")} *Nenhuma chave ativa*`;
    const container = createContainer(color, createSection({
        ...
        .setEmoji(getEmojiId("action_remove") || "🔄"), new ButtonBuilder()
        .setCustomId("my_payment/refresh")
        .setLabel("Atualizar")
        .setStyle(ButtonStyle.Secondary)
        .setEmoji(getEmojiId("clock") || "🔄")), Separator.Default, `🔒 *Painel Privado: Suas credenciais são protegidas. Ninguém além de você tem acesso a esta tela.*`);
    return container;
}
// ==========================================
...
createCommand({
    name: "meu-pagamento",
    description: "💳 Configure suas chaves individuais de pagamento (PIX, Mercado Pago, Stripe).",
    ...
    async run(interaction) {
        ...
        const isStaff = guildData.channels?.staffRole &&
        ...
        const container = await renderMyPaymentContainer(interaction.user.id, interaction.guildId, interaction.client);
        await interaction.reply({
...
createResponder({
    ...
    async run(interaction) {
        ...
        const typeInput = new TextInputBuilder()
            ...
            .setStyle(TextInputStyle.Short)
            .setRequired(false);
        modal.addComponents(new LabelBuilder().setLabel("Nome do Recebedor:").setTextInputComponent(nameInput), new LabelBuilder().setLabel("Chave PIX:").setTextInputComponent(keyInput), new LabelBuilder()
...
createResponder({
    ...
    async run(interaction) {
        ...
        const rawType = interaction.fields.getTextInputValue("pix_type")?.trim() || "Chave PIX";
        if (!rawName) {
            await interaction.reply({
        ...
        const container = await renderMyPaymentContainer(interaction.user.id, interaction.guild.id, interaction.client);
