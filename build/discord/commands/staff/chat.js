    ...
    async run(interaction) {
        ...
        if (subcommand === "desbloquear") {
            try {
                ...
                const container = createContainer(constants.colors.primary, `| ${getEmojiTag("unlock")} **Chat Desbloqueado:**\nEste canal foi liberado por <@${interaction.user.id}>. Todos os membros podem digitar novamente.`);
                await interaction.reply({
                    components: [container],
                    flags: ["IsComponentsV2"],
                });
        ...
        if (subcommand === "limpar") {
            ...
            try {
                ...
                const container = createContainer(constants.colors.primary, `| ${getEmojiTag("file_remove")} **Limpeza Concluída:**\nForam apagadas com sucesso \`${deleted.size}\` mensagens deste canal.`);
