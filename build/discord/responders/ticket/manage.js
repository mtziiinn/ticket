    ...
    return createContainer(color, createSection({
        content: `## <:other_ticket:1502789959378145300> Ticket ${ticket.ticketId}\n${owner || "Usuário"} Seja bem-vindo(a) ao seu ticket! Através deste canal, a equipe irá realizar seu atendimento e esclarecer suas dúvidas.` +
            (isClaimed
                ...
                : ""),
        thumbnail: getCleanAvatarURL(owner?.user || owner),
        ...
        switch (action) {
            case "claim": {
                ...
                await channel
                    ...
                    .catch((err) => console.error("[Manage] Erro ao enviar aviso no canal:", err));
                // Enviar Log de Ação
                ...
                const targetOwner = (await interaction.client.users.fetch(ticket.ownerId).catch(() => null)) ||
                    owner?.user;
                if (targetOwner) {
                    ...
                    const color = guildData?.identity?.primaryColor
                        ? formatHexColor(guildData.identity.primaryColor)
                        : constants.colors.azoxo;
                    const dmContainer = createContainer(color, createSection({
            ...
            case "close_modal": {
                ...
                const transcriptLabel = new LabelBuilder()
                    ...
                    .setRadioGroupComponent(new RadioGroupBuilder()
                    ...
                    if (existing.status === "completed") {
                        ...
                        if (targetOwner && ticket) {
                            const dmContainer = createContainer(constants.colors.primary, createSection({
                                ...
                                thumbnail: getCleanAvatarURL(user),
                            }), Separator.Default, `<:file_add:1502789905112105071> **Arquivo:** \`${existing.filename}\``, `<:clipboard:1502789887907205293> **Descrição:** ${existing.description}`, `<:cloud_check:1502789867355115690> **Link:** ${existing.url}`, Separator.Default, `<:action_warning:1502789801949265990> O link expira em **7 dias**.`, createRow(new ButtonBuilder({
                ...
                if (logChannelId) {
                    ...
                    if (logChannel?.isTextBased()) {
                        ...
                        const logContainer = createContainer(constants.colors.primary, createSection({
                            ...
                            thumbnail: getCleanAvatarURL(owner?.user || interaction.client.user),
        ...
        try {
            ...
            if (owner) {
                const dmContainer = createContainer(constants.colors.azoxo, createSection({
                    ...
                    thumbnail: getCleanAvatarURL(guild.iconURL({ extension: "png", forceStatic: true }) ||
...
async function getOrCreateVaultWebhook(vaultChannel, clientUser) {
    ...
    try {
        ...
        if (webhooks && webhooks.size > 0) {
            ...
            const existing = webhooks.find((w) => w.owner?.id === clientUser?.id) ||
                webhooks.find((w) => w.name === "Cofre de Mídia" || w.name === "Ticket Media Vault") ||
                webhooks.first();
            if (existing) {
...
export async function generateTranscript(channel, ticket, closer) {
    try {
        ...
        if (!vaultChannel && channel.guild) {
            vaultChannel = channel.guild.channels.cache.find((c) => c.isTextBased() &&
                /^(imagens|cofre|vault|backup-midia|fotos|midia|logs-ticket)$/i.test(c.name));
        ...
        let vaultWebhook = null;
        if (vaultChannel?.isTextBased()) {
            vaultWebhook = await getOrCreateVaultWebhook(vaultChannel, channel.client?.user);
        }
        ...
        const claimerMember = ticket.claimedBy && channel.guild
            ...
            : null;
        const claimerUser = claimerMember?.user ||
            (ticket.claimedBy
                ...
                for (const att of msg.attachments.values()) {
                    ...
                    if (vaultWebhook) {
                        try {
                            ...
                            console.error(`[Vault Webhook] Erro ao enviar anexo via webhook:`, err);
        ...
        const transcriptData = {
            ...
            openedBy: {
                ...
                avatar: ownerUser?.displayAvatarURL?.({ extension: "png", forceStatic: true }) ||
            ...
            closedBy: {
                ...
                avatar: closer?.displayAvatarURL?.({ extension: "png", forceStatic: true }) ||
                    "https://cdn.discordapp.com/embed/avatars/0.png",
            },
            claimedBy: claimerUser
                ? {
                    ...
                    avatar: claimerUser.displayAvatarURL?.({ extension: "png", forceStatic: true }) ||
                        "https://cdn.discordapp.com/embed/avatars/0.png",
                }
                : undefined,
        ...
        console.log(`[Transcript] Salvo com sucesso no banco: ID ${transcriptId} (${sortedMessages.length} mensagens)`);
