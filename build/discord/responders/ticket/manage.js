        ...
        switch (action) {
            ...
            case "send_pix": {
                ...
                if (pixMessage) {
                    ...
                    setTimeout(async () => {
                        try {
                            console.log(`[Manage] Iniciando busca de mensagem de pin PIX em: ${channel.id}`);
                            const messages = await channel.messages.fetch({
                                limit: 50,
                            });
                            console.log(`[Manage] Mensagens buscadas: ${messages.size}`);
                            const pinSystemMessage = messages.find((m) => {
                                console.log(`[Manage] Verificando msg ${m.id} - Tipo: ${m.type}`);
                                return m.type === MessageType.ChannelPinnedMessage;
                            });
                            if (pinSystemMessage) {
                                console.log(`[Manage] Mensagem de pin encontrada (${pinSystemMessage.id}). Tentando deletar...`);
                                await pinSystemMessage
                                    .delete()
                                    .then(() => console.log(`[Manage] Mensagem de pin deletada com sucesso.`))
                                    .catch((err) => console.error(`[Manage] Erro ao deletar msg de pin:`, err));
                            }
                            else {
                                console.log(`[Manage] Nenhuma mensagem de pin encontrada nas últimas 50 mensagens.`);
                            }
                        }
                        catch (e) {
                            console.error("[Manage] Erro crítico ao processar limpeza de pin PIX:", e);
                        }
                    }, 5000);
                }
