...
import { ButtonBuilder, ButtonStyle, ModalBuilder, TextInputBuilder, TextInputStyle, StringSelectMenuBuilder, } from "discord.js";
import { db } from "#database";
export async function createConfigPanel(guildId) {
    ...
    const catDisplay = customCats.length > 0
        ? customCats
            .map((c) => {
            const emojiDisplay = c.emoji?.length && c.emoji.length > 5 && !c.emoji.includes(":")
                ? `<:emoji:${c.emoji}>`
                : c.emoji || "🎫";
...
createResponder({
    ...
    async run(interaction, { action }) {
        ...
        if (action === "channels") {
            ...
            modal.addComponents(createRow(new TextInputBuilder()
                .setCustomId("logs")
                .setLabel("ID do Canal de Logs")
                .setValue(guildData.channels?.tickets || "")
                .setStyle(TextInputStyle.Short)
                .setRequired(true)), createRow(new TextInputBuilder()
                .setCustomId("vault")
        ...
        if (action === "cat_add") {
            ...
            modal.addComponents(createRow(new TextInputBuilder()
                ...
                .setStyle(TextInputStyle.Short)
                .setRequired(false)), createRow(new TextInputBuilder()
...
createResponder({
    ...
    async run(interaction) {
        ...
        if (guildData.channels) {
            guildData.channels.tickets = data.logs;
            guildData.channels.vault = data.vault;
            guildData.channels.staffRole = data.staffRole;
            guildData.channels.pixKey = data.pixKey;
        }
        guildData.markModified("channels");
        await guildData.save();
        const panel = await createConfigPanel(guildId);
...
createResponder({
    ...
    async run(interaction) {
        ...
        if (guildData.channels?.ticketCategories) {
            guildData.channels.ticketCategories =
                guildData.channels.ticketCategories.filter((c) => c.value !== valueToRemove);
            guildData.markModified("channels");
            await guildData.save();
        }
...
createResponder({
    ...
    async run(interaction) {
        ...
        const slug = values[0];
        const guildData = await db.guilds.get(guildId);
        const cat = guildData.channels?.ticketCategories?.find((c) => c.value === slug);
        if (!cat) {
            await interaction.reply({
...
createResponder({
    ...
    async run(interaction) {
        ...
        if (guildData.channels?.ticketCategories) {
            const index = guildData.channels.ticketCategories.findIndex((c) => c.value === oldSlug);
            if (index !== -1) {
                const rawSlug = data.slug;
