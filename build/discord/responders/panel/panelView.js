...
export async function renderHomeTab(guild, client) {
    ...
    return createContainer(PANEL_COLOR, "## PAINEL DE CONFIGURAÇÕES", buildPanelDropdown("home"), Separator.Default, `| **Status do BOT:** \`🟢 Online - ${ping}ms\``, Separator.Default, `| **Tickets em Aberto:** \`${openTicketsCount}\``, Separator.Default, `| **Membros no Servidor:** \`${memberCount}\``, Separator.Default, [
        `### 🔔 Avisos Importantes`,
...
export async function renderTicketTab(guildData) {
    ...
    return createContainer(PANEL_COLOR, "## 📁 Sistema de Ticket", buildPanelDropdown("ticket"), Separator.Default, createRow(new ButtonBuilder()
        .setCustomId("panel/ticket/send_panel")
        .setLabel("Enviar Painel do Ticket")
        .setStyle(ButtonStyle.Success)
        .setEmoji("📨")), Separator.Default, createSection({
...
export async function renderPaymentsTab(guildData) {
    ...
    return createContainer(PANEL_COLOR, "## 💳 Sistema de Pagamentos", buildPanelDropdown("payments"), Separator.Default, createSection({
...
export async function renderAutoroleTab(guildData) {
    ...
    return createContainer(PANEL_COLOR, "## 👥 Sistema de Boas-vindas & Autorole", buildPanelDropdown("autorole"), Separator.Default, createSection({
...
export async function renderVerificationTab(guildData) {
    ...
    return createContainer(PANEL_COLOR, "## 🛡️ Sistema de Verificação (Captcha)", buildPanelDropdown("verification"), Separator.Default, createRow(new ButtonBuilder()
...
export async function renderLogsTab(guildData) {
    const logsChannel = guildData.botLogsChannel
        ...
        : "*Não configurado*";
    return createContainer(PANEL_COLOR, "## 📁 Sistema de Logs do Discord", buildPanelDropdown("logs"), Separator.Default, createSection({
...
export async function renderIdentityTab(guild) {
    return createContainer(PANEL_COLOR, "## 🎨 Identidade Visual do BOT", buildPanelDropdown("identity"), Separator.Default, `| **Servidor:** ${guild.name}\n| **Cor Principal:** \`${PANEL_COLOR}\` (Verde Neon)\n| **Banner Atual:** Visual padrão configurado`, Separator.Default, createMediaGallery(BANNER_URL));
}
export async function renderSalesTab() {
    return createContainer(PANEL_COLOR, "## 🛒 Sistema de Vendas", buildPanelDropdown("sales"), Separator.Default, `| **Status das Vendas:** Sistema integrado e pronto para cobranças via \`/gerar-pagamento\`.\n| **Gateways:** Suporte a PIX com QR Code, Mercado Pago e Stripe.`, Separator.Default, `*Para emitir cobranças diretas para clientes em um canal de ticket ou chat, use o comando \`/gerar-pagamento\`.*`);
}
export async function renderCommandsTab() {
    return createContainer(PANEL_COLOR, "## ⌨️ Comandos do BOT", buildPanelDropdown("commands"), Separator.Default, [
