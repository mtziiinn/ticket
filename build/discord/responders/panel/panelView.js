...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelSelectMenuBuilder, ChannelType, RoleSelectMenuBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, } from "discord.js";
...
export async function renderHomeTab(guild, client, guildData) {
    if (!guildData) {
...
export async function renderTicketTab(guildData) {
    ...
    return createContainer(color, `## ${getEmojiTag("prism") || getEmojiTag("other_ticket")} Sistema de Ticket`, buildPanelDropdown("ticket"), Separator.Default, createRow(new ButtonBuilder()
        ...
        button: new ButtonBuilder()
            ...
            .setStyle(ButtonStyle.Secondary)
            .setEmoji(getEmojiId("action_add") || "✏️"),
    }), Separator.Default, Separator.Default, createSection({
        ...
        .setStyle(ButtonStyle.Secondary)
        .setEmoji(getEmojiId("action_add") || "✏️")
        .setDisabled(categories.length === 0), new ButtonBuilder()
...
export async function renderAutoroleTab(guildData) {
    ...
    return createContainer(color, `## ${getEmojiTag("user_users")} Sistema de Boas-vindas & Autorole`, buildPanelDropdown("autorole"), Separator.Default, createSection({
...
export async function renderLogsTab(guildData) {
    ...
    const logsChannel = guildData.botLogsChannel
        ...
        : "*Não configurado*";
    return createContainer(color, `## ${getEmojiTag("folder")} Sistema de Logs do Discord`, buildPanelDropdown("logs"), Separator.Default, createSection({
...
export async function renderIdentityTab(guild, client, guildData) {
    if (!guildData) {
        guildData = await db.guilds.get(guild.id);
    }
...
export async function renderAntifloodTab(guildData) {
    const color = getPanelColor(guildData);
    ...
    return createContainer(color, `## ${getEmojiTag("shield")} Sistema Anti-Flood da Equipe`, buildPanelDropdown("antiflood"), Separator.Default, createSection({
        ...
        button: new ButtonBuilder()
            ...
            .setStyle(ButtonStyle.Secondary)
            .setEmoji(getEmojiId("action_add") || "⚙️"),
    }), Separator.Default, `*💡 Dica: Membros com permissão de Administrador ou com o cargo da equipe são imunes à punição.*`);
}
export function renderAnunciosTab(guildData) {
    const color = getPanelColor(guildData);
    ...
    return createContainer(color, `## ${getEmojiTag("prism")} Sistema de Disparo de Anúncios & Comunicados`, buildPanelDropdown("anuncios"), Separator.Default, `> ${getEmojiTag("prism")} **Canal Padrão:** ${targetChannel}\n` +
        ...
        .setMinValues(1)
        .setMaxValues(25)), createRow(new ButtonBuilder()
        ...
        .setStyle(ButtonStyle.Primary)), createRow(new ButtonBuilder()
